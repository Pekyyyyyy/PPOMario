...
from mario_rl.ddqn.config import EnvConfig
from mario_rl.utils import reset_env, seed_everything, step_env


class StepCompatWrapper(gym.Wrapper):
    """Normalize step() output to 5-tuple (obs, reward, terminated, truncated, info)
    regardless of whether the inner env returns 4 or 5 values."""
    ...
    def step(self, action):
        ...
        for _ in range(self.skip):
            ...
            if len(result) == 5:
                ...
                observation, reward, term, info = result
                terminated = term
            total_reward += reward
...
def build_env(config: EnvConfig) -> gym.Env:
    ...
    env = StepCompatWrapper(env)  # normalize 4→5 tuple for downstream gymnasium wrappers
